...
export const useScheduler = () => {
    ...
    const calculateSchedule = useCallback((config) => {
        ...
        const refineSchedule = (arr) => {
            const refined = [...arr];
            for (let i = 0; i < refined.length; i++) {

                if (refined[i] === TYPES.I && i > 0) {
                    if (refined[i - 1] === TYPES.I || refined[i - 1] === TYPES.P || refined[i - 1] === TYPES.B) {
                        refined[i] = refined[i - 1] === TYPES.P ? TYPES.P : TYPES.D;
                    }
                }

                if (refined[i] === TYPES.S && i > 0) {
                    if (refined[i - 1] === TYPES.S || refined[i - 1] === TYPES.I) {
                        refined[i] = TYPES.D;
                    }
                }
                ...
                if (refined[i] === TYPES.S) {
                    ...
                    while (lookAhead < refined.length && refined[lookAhead] !== TYPES.S) {
                        if (refined[lookAhead] === TYPES.P) pCount++;
                        if (refined[lookAhead] === TYPES.B && firstBIndex === -1) firstBIndex = lookAhead;
                        lookAhead++;
                    }

                    if (pCount === 0 && firstBIndex !== -1) {
                        refined[firstBIndex] = TYPES.P;
                        pCount++;
                    }

                    if (pCount === 1) {
                        const onlyP = refined.indexOf(TYPES.P, i);
                        if (onlyP !== -1 && onlyP + 1 < refined.length && refined[onlyP + 1] !== TYPES.S) {
                            refined[onlyP + 1] = TYPES.P;
                        }
                    }
                }
            }
        ...
        const fs1 = refineSchedule(s1);
        const fs2 = refineSchedule(s2);
        const fs3 = refineSchedule(s3);
